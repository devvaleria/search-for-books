...
function SearchForm({ APIKey }: defaultPropsType) {
  const categories: Array<categoryType> = useAppSelector(
    (state) => state.search.categories
  );
  const sortValues: Array<sortType> = useAppSelector(
    (state) => state.search.sortValues
  );
  ...
  const [selectedSortingBy, setselectedSortingBy] =
    ...
    <div className={style.searchForm}>
      <div className={style.inTitleInput}>
        <input
          ...
          className={style.inTitleInput_input}
          type="text"
        />
        <button
          onClick={() => {
        ...
        >
          <img className={style.inTitleInput_btnImg} src={searchIcon} />
        </button>
      </div>
